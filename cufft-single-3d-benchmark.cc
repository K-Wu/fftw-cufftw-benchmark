...
static void cu_fft_single_3d(benchmark::State &state)
{
  int N = state.range(0);

  // Allocate host memory for the signal and result
  cufftComplex *h_signal = (cufftComplex *)malloc(sizeof(cufftComplex) * N * N * N);
  cufftComplex *h_fft = (cufftComplex *)malloc(sizeof(cufftComplex) * N * N * N);

  //  Allocate complex signal GPU device memory
  cufftComplex *d_signal;
  checkCudaErrors(cudaMalloc((void **)&d_signal, N * N * N * sizeof(cufftComplex)));

  //  Init CUFFT Plan
  cufftHandle plan;
  checkCudaErrors(cufftPlan3d(&plan, N, N, N, CUFFT_C2C));
  ...
  for (auto _ : state)
  {
    ...
    checkCudaErrors(cudaMemcpy(d_signal, h_signal, N * N * N * sizeof(cufftComplex), cudaMemcpyHostToDevice));
  ...
  state.SetBytesProcessed(static_cast<int64_t>(state.iterations()) * N * N * N * sizeof(cufftComplex));
  state.SetComplexityN(N);
}
BENCHMARK(cu_fft_single_3d)->RangeMultiplier(2)->Range(1 << 3, 1 << 12)->Complexity()->UseManualTime();
BENCHMARK_MAIN();
